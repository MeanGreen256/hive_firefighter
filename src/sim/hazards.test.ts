...
describe('propane hazards', () => {
  it('starts a countdown from cell heat and lets water reset it', () => {
    ...
    const events = advanceHazards(hazards, fire, DEFAULT_FIRE_SIMULATION_TUNING, 1);
  ...
  it('does not silently save an announced countdown when its heat source burns out', () => {
    const fire = createFireSimulation(createCellGrid({ width: 3, height: 1, depth: 3 }));
    const hazards = createHazardSimulation(placement);
    ...
    expect(tank.heat).toBe(PROPANE_COUNTDOWN_HEAT + 1);
  });
  ...
  it('fails into a blast that destroys cells, spreads fire, and emits one event', () => {
    ...
    const events = advanceHazards(hazards, fire, DEFAULT_FIRE_SIMULATION_TUNING, 0.1);
    const failure = events.find((event) => event.type === IncidentEventType.PropaneFailed);

    expect(failure).toMatchObject({ hazardId: 'tank' });
    ...
    expect(advanceHazards(hazards, fire, DEFAULT_FIRE_SIMULATION_TUNING, 1)).toEqual([]);
  });
});
